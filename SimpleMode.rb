...
def SimpleMode(arr)
    ...
    #get count of all integers in array then get max
    digits_with_count = {}

    arr.map{|c| digits_with_count[c] = arr.count(c)}
  
    digits_with_count.key(digits_with_count.values.max)
end
print SimpleMode([10, 4, 5, 2, 4, 5, 5])